...
namespace Ergo
{
    internal interface IChainableResult
    {
        ...
        Result<TOut> OnSuccess<TOut>(Func<TOut> mapper); // <- Result.Success(TOut)
        AsyncResult<TOut> OnSuccess<TOut>(Func<Task<TOut>> mapper); // <- Result.Success(TOut)
        ...
        Result<TOut> OnFailure<TOut>(Func<Result, TOut> mapper); // <- Result.Success(TOut)
        AsyncResult<TOut> OnFailure<TOut>(Func<Result, Task<TOut>> mapper); // <- Result.Success(TOut)
    }
}